...
def solicitar_numero_flotante(mensaje, positivo=True):
    while True:
        ...
        if entrada.count('.') <= 1 and entrada.replace('.', '').isnumeric():
            ...
            print("Ha ingresado un número inválido. Por favor intente de nuevo")
...
def registrar_evento(): # basado en el Ejercicio 3, fase 2
    ...
    while True:
        while True:
            ...
            if not (titulo in titulos_eventos):
                break
            else:
        ...
        while True:
            ...
            if evento_mayores != "1" and evento_mayores != "2":      #Iniciamos un ciclo condicional para validar la opción ingresada por el usuario y definir si el evento es solo para mayores de edad o no
        ...
        espacios_totales = solicitar_numero_entero("Cantidad total de espacios: ")
        ...
        precio_vip = solicitar_numero_flotante("Precio de boleto VIP: ", True)
        print("El precio del boleto VIP debe ser mayor o igual a 0.")
        precio_preferencial = solicitar_numero_flotante("Precio de boleto Preferencial: ", True)
        while precio_preferencial < 0:                          #Iniciamos un ciclo condicional para validar que el precio del boleto Preferencial ingresado por el usuario sea mayor o igual a 0
            ...
            precio_preferencial = solicitar_numero_flotante("Precio de boleto Preferencial: ", True)
        precio_general = solicitar_numero_flotante("Precio de boleto General: ", True)
        ...
        while True:
            ...
            print("1. Sí")
            print("2. No")
            evento_adicional = input("Seleccione una opción: ")
            if evento_adicional not in ["1", "2"]:
...
def modificar_evento(): # Funcion para editar los eventos de la Lista, submenú Admin
    ...
    for evento in eventos_disponibles:  #Iniciamos un ciclo para recorrer la lista de eventos disponibles
        ...
        for entrada in entradas_compradas:
            if entrada['titulo'].strip().lower() == evento['titulo'].strip().lower():
                espacios_intocables = espacios_intocables + entrada['cantidad_entradas']
        ...
        if evento['titulo'].strip().lower() == titulo.strip().lower():  #Si el título del evento ingresado por el usuario coincide con algún evento en la lista de eventos disponibles
            ...
            while True:
                nuevos_espacios_totales = solicitar_numero_entero("Ingrese la nueva cantidad total de espacios (digite 0 para no cambiar): ")  #Solicitamos al usuario que ingrese la nueva cantidad total de espacios, si desea cambiarla
                if nuevos_espacios_totales >= espacios_intocables or nuevos_espacios_totales == 0:
                    break
                else:
                    print('No se pueden reducir la cantidad de entradas a una cantidad menor a las entradas ya vendidas')
            if nuevos_espacios_totales > 0:  #Si el usuario ingresó una nueva cantidad total de espacios
                evento['espacios_totales'] = nuevos_espacios_totales  #Actualizamos la cantidad total de espacios en la lista de eventos disponibles
            ...
            while True:
                ...
                if nuevos_espacios_disponibles > 0:  #Si el usuario ingresó una nueva cantidad de espacios disponibles
                    if evento['espacios_totales'] > espacios_intocables + nuevos_espacios_disponibles:
                        ...
                        print('La cantidad de nuevos espacios disponibles no pueden superar los espacios totales del evento')



            nuevo_precio_vip = solicitar_numero_flotante("Ingrese el nuevo precio de boleto VIP (Digite 0 para no cambiar): ")  #Solicitamos al usuario que ingrese el nuevo precio del boleto VIP, si desea cambiarlo
            if nuevo_precio_vip > 0:  #Si el usuario ingresó un nuevo precio del boleto VIP
                evento['precio_vip'] = nuevo_precio_vip  #Actualizamos el precio del boleto VIP en la lista de eventos disponibles
            nuevo_precio_preferencial = solicitar_numero_flotante("Ingrese el nuevo precio de boleto Preferencial (Digite 0 para no cambiar): ")  #Solicitamos al usuario que ingrese el nuevo precio del boleto Preferencial, si desea cambiarlo
            if nuevo_precio_preferencial > 0:  #Si el usuario ingresó un nuevo precio del boleto Preferencial
                evento['precio_preferencial'] = nuevo_precio_preferencial  #Actualizamos el precio del boleto Preferencial en la lista de eventos disponibles
            nuevo_precio_general = solicitar_numero_flotante("Ingrese el nuevo precio de boleto General (Digite 0 para no cambiar): ")  #Solicitamos al usuario que ingrese el nuevo precio del boleto General, si desea cambiarlo
            if nuevo_precio_general > 0:  #Si el usuario ingresó un nuevo precio del boleto General
                evento['precio_general'] = nuevo_precio_general  #Actualizamos el precio del boleto General en la lista de eventos disponibles
        ...
        for entrada in entradas_compradas:
            if entrada['titulo'].strip().lower() == nombre.strip().lower():
                ...
                if tipo_entrada == "general":
                    ...
                    vendidas_preferencial += cantidad_entradas
                elif tipo_entrada == "vip":
                    vendidas_vip += cantidad_entradas
    ...
    for evento in eventos_disponibles:  #Iniciamos un ciclo para recorrer la lista de eventos disponibles
        if evento['titulo'].strip().lower() == titulo.strip().lower():  #Si el título del evento ingresado por el usuario coincide con algún evento en la lista de eventos disponibles
            ...
            while True:
                ...
                cantidad_entradas = solicitar_numero_entero("Cantidad de entradas: ")
            ...
            efectivo = solicitar_numero_flotante("Efectivo: ", True)
            ...
            if efectivo < precio_total:  #Validamos que la cantidad de efectivo ingresada por el usuario sea mayor o igual al precio total de las entradas
                print(f"No tiene suficiente efectivo. El precio total es: {precio_total}")  #Si la cantidad de efectivo ingresada por el usuario no es suficiente, mostramos un mensaje indicando que no tiene suficiente efectivo y mostramos el precio total de las entradas
                return  #Salimos de la función
                ...
                entradas_compradas.append(
                    {
                        'titulo': titulo,
                        'evento_para_mayores': True if evento['evento_para_mayores'] == True else False,
                        'tipo_entrada': tipo_entrada,
                        'cantidad_entradas': cantidad_entradas
                    }
                )  #Agregamos la compra a la lista de entradas compradas
...
entradas_compradas = [
    {
        ...
        'evento_para_mayores': True,
        'tipo_entrada': "VIP",
        'cantidad_entradas': 1 # Contador que indica cuantas entradas se adquirieron en la transacción
    },
    {
        'titulo': "Coldplay",
        'evento_para_mayores': False,
